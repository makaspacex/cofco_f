...
layui.define(['table'], function (exports) {
  ...
  //获得数据
  table.Class.prototype.pullData = function (curr, refresh, showloading=true) {
    var that = this
      , options = that.config
  ...
  table.reload = function (tableId, config, shallowCopy) {
    ...
    if (smartReload.enable() && configTemp.smartReloadModel) {
      ...
      if (!reloadModel) {
        ...
        if(configTemp.reloaddingShow){
          insTemp.loading();
        }
        insTemp.pullData(insTemp.page,false,configTemp.reloaddingShow);
