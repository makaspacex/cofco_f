...
.layui-form-item .layui-input-inline{max-width:90%;width:auto;min-width:300px;}
...
<form action="{:url('?group='.input('param.group', 'base'))}" class="page-list-form layui-form layui-form-pane" method="post">
    ...
    {switch name="v['type']"}
        ...
        {case value="multi_textarea"}
        <!--富文本-->
        <div class="layui-form-item" >
            <label class="layui-form-label">{$v['title']}</label>
            <div class="layui-input-inline" style="width: 80%">
                <textarea rows="{:config('sys.multi_textarea_num')}"  id="id_{$v.id}" name="id[{$v['id']}]" >
                {:htmlspecialchars_decode($v['value'])}</textarea>
                <br>
                <div class="layui-form-mid layui-word-aux" style="width: 100% ">{:htmlspecialchars_decode($v['tips'])}
                    <br>调用方式：
                    <code>config('{if condition="isset($v['module'])"}module_{/if}{:input('param.group', 'base')}.{$v['name']}')</code>
                </div>
            </div>
            {:editor(['id_'.$v['id']])}
        </div>
